...
class Vcard_GUI:
    def __init__(self):
        ...
        toolmenu.add_command(
            label="Remove Finnish area code", command=self.remove_area_code
        )
        ...
        self.__search_button = Button(
            self.__search_frame, text="Search", command=self.search
        )
        self.__search_entry = Entry(self.__search_frame)
        self.__clear_search_button = Button(
            self.__search_frame, text="X", command=self.clear_search
        )
        ...
        self.__main_window.bind(
            "<Control-f>", lambda event: self.__search_entry.focus()
        )
    ...
    def edit_text(self, event=None, new=False):
        ...
        def want_to_save(event=None):
            ...
            want_save = messagebox.askyesno(
                "Oops", "Want to save?", parent=self.__edit_window
            )
            if want_save:
                ok_button.invoke()
            else:
    ...
    def search(self, event=None):
        ...
        for contact in self.__contacts:
            ...
            if (
                search_word in search_string
                or search_word.capitalize() in search_string
                or search_word.upper() in search_string
                or search_word.lower() in search_string
            ):
